...
//Step 3: Add item model as a disabled child of the item gameObject, and then place the model into the itemModel array in the same position as the itemID.
//Step 4: Give the item model a collider.
...
public class ItemValue : MonoBehaviour
{
    ...
    //Reference to the currently used item model
    ...
    void AssignItem(int newID)
    {
        ...
        EnableModel();
        //Enable the model's collision shape
    ...
    void EnableModel()
    {
        for (int i = 0; i < itemModels.Length; i++) //Repeat for every model
        {
            if (i != id) //Check current ID
            {
                ...
                currentItemModel = itemModels[i];
            }
        }
    }


    //Enable the correct model's collision component and disable all other model's collision components
    void EnableModelCollision()
    {
        for (int i = 0; i < itemModels.Length; i++) //Repeat for every model
        {
            SphereCollider sphereCollider;
            if (itemModels[i].TryGetComponent<SphereCollider>(out sphereCollider)){
